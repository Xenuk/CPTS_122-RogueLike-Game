...
void Game::runGame()
{
	...
	while (window->isOpen() && gameState)
	{
		...
		if (window->hasFocus())
		{
			if (sf::Mouse::isButtonPressed(sf::Mouse::Button::Left))
			{	
				// if (newGameguy->getCurrWeapon() && (newGameguy->getCurrWeapon()->getAmmo() == -1 || newGameguy->getCurrWeapon()->getCurrAmmo() > 0))
...
// 	{
// 		p->move(p->directionAndSpeed);
// 	}

// }

// haven't implemented these setters and getters from merge yet
void Game::projectileHandling()
{
	for (int i = static_cast<int>(projectiles.size()) - 1; i >= 0; --i)
	{
