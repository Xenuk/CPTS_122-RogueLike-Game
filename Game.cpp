...
void Game::escapeMenu(bool &loopVariable) // do relative camera menu.
{
  ...
  playButton.setPosition(
      {gameObjects[1]->getPosition().x, gameObjects[1]->getPosition().y - 50});
  playButton.setScale({1, 1});

  sf::Texture settingsButtonTexture =
      createTexture("Sprites/SettingsButton.png");
  sf::Sprite settingsButton(settingsButtonTexture);
  settingsButton.setOrigin({47, 10});
  settingsButton.setPosition(
      {gameObjects[1]->getPosition().x,
       gameObjects[1]->getPosition().y}); // gets relative position.
  ...
  while (window->isOpen() && loop) {
    ...
    sf::Vector2f mousePos = window->mapPixelToCoords(
        sf::Mouse::getPosition(*window)); // lotsa reused code.
    if (window->hasFocus()) {
...
void Game::mainMenu() {
  createWindow(320,
               180); // creates 1920x1080 window with a view that is 320 by 180.

  sf::Time elapsed1 = globalClock.getElapsedTime();
  std::cout << "Main Menu Opened At: " << elapsed1.asSeconds() << "s"
            << std::endl; // Functional Running Time Clock
  ...
  while (window->isOpen() && !exit) {
    window->setView(sf::View(
        {960, 540}, {static_cast<float>(1920), static_cast<float>(1080)}));
...
void Game::guiInterface() {
  ...
  if (guiInterfaceArray[0] == nullptr) {
    ...
    sf::Text *time = new sf::Text(*fontArray[0], "N/A", 16);
    if (9 - static_cast<int>(timeClock.getElapsedTime().asSeconds() / 60) <=
        0) {
      minutes = std::to_string(0) + ":";
    } else {
      minutes =
          std::to_string(9 - static_cast<int>(
                                 timeClock.getElapsedTime().asSeconds() / 60)) +
          ":";
    }
    if (secs - static_cast<int>(timeClock.getElapsedTime().asSeconds()) < 10) {
      seconds =
          "0" +
          std::to_string(
              secs - static_cast<int>(timeClock.getElapsedTime().asSeconds()));
    } else {
      seconds = std::to_string(
          secs - static_cast<int>(timeClock.getElapsedTime().asSeconds()));
    }
    time->setString(minutes + seconds);
    time->setPosition({gameObjects[1]->getPosition().x - 10,
                       gameObjects[1]->getPosition().y - 80});
    time->setScale({0.4, 0.4});
    guiInterfaceArray[0] = time;
  } else if (guiInterfaceArray[0] != nullptr) {
    ...
    guiInterfaceArray[0]->setString(minutes + seconds);
    guiInterfaceArray[0]->setPosition({gameObjects[1]->getPosition().x - 10,
                                       gameObjects[1]->getPosition().y - 80});
  }
}
Game::Game() {}
...
void Game::runGame() {
  ...
  const float dt =
      1.0f / 60.0f; // had to use ai for this due to it being jittery as heck.
  float accumulator =
      0.0f; // all the time calc except for the clock.getElapsedTime stuff, was
  ...
  sf::Time elapsed1 = globalClock.getElapsedTime();
  std::cout << "Game Running Start Time: "
            << globalClock.getElapsedTime().asSeconds() << "s" << std::endl;
  ...
  sf::Texture texture2 = createTexture(
      "Sprites/ExampleSpriteWall.png"); // static allocation so perhaps make it
  ...
  GameObject *newWallGuy = new GameObject(
      map, 10, 10, 10, 0, 0, pistol); // TODO: make walls not have weapons
  GameObject *newEnemyGuy =
                        ...
                        unsigned int nHeight) // This is the actual game start
{
  ...
  sf::RenderWindow *newWindow =
      new sf::RenderWindow(sf::VideoMode({1920, 1080}), "Game");
  std::cout << "Window Rendered: Height = " << height << ", Width = " << width
            << ". " << std::endl;
